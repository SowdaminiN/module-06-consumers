...
def send_temp(host: str, queue_name1: str, queue_name2: str, queue_name3: str):
    """
    ...
    for row in reader:
        ...
        try:
            ...
            try:
                Smoker = round(float(Smoker_temp),1)
                # create a message from our data
                smoker = f"Smoker:{Smoker}"
            ...
            try:
                FoodA = round(float(FoodA_temp),1)
                # use an fstring to create a message from our data
                Food_A = f"FoodA:{FoodA}"
                # prepare a binary (1s and 0s) message to stream
            ...
            try:
                FoodB = round(float(FoodB_temp),1)
                # use an fstring to create a message from our data
                # notice the f before the opening quote for our string?
                Food_B = f"FoodB:{FoodB}"
                # prepare a binary (1s and 0s) message to stream
